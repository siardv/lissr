---
title: "Multi-Module Linkage"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Multi-Module Linkage}
...
`wave_year`, you get `NA` in modules that did not have a wave that
year. This is expected, not an error.
...
primary outcome, the others are predictors), use `left_join()` so you
keep only person-years present in Health.
...
- **Check each module's merge report** for comparability contracts
  ...
  boundaries.
- **Carry forward era flags** into the linked dataset. They are
  present in each module's merged output and transfer through the join.
...
- [ ] Merged each module independently with its own recipe before
  joining.
- [ ] Renamed ambiguous columns (e.g. `s001` means different things in
  different modules) before the join.
- [ ] Used `wave_year` (not `wave_id`) as the time key for cross-module
  joins — wave IDs are module-specific strings.
